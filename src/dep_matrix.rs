...
impl<'a> PartialEq for DepMatrix<'a> {
    fn eq(&self, other: &Self) -> bool {
        self.side == other.side && self.matrix == other.matrix
    }
}


pub fn print_unneeded_no_matrix(db: &Db, alpm_packages: AlpmList<&Package>) {
    let pkg_count = alpm_packages.len();
    ...
    let mut dep_hash_to_pkg: HashMap<DepHashOpt, PackageIndex> = HashMap::with_capacity(pkg_count);
    ...
    for &pkg in &index_to_pkg {
        ...
        // Push its dependencies onto the stack for DFS
        push_dependencies(db, &pkg_to_index, &dep_to_providers, &mut dep_hash_to_pkg, &visited, &mut stack, pkg);
        ...
        while let Some(pkg_index) = stack.pop() {
            ...
            push_dependencies(db, &pkg_to_index, &dep_to_providers, &mut dep_hash_to_pkg, &visited, &mut stack, index_to_pkg[pkg_index.0]);
        }
    }
...
pub fn print_unneeded_no_matrix_no_hash_opt(db: &Db, alpm_packages: AlpmList<&Package>) {
    let pkg_count = alpm_packages.len();
    ...
    for &pkg in &index_to_pkg {
        ...
        // Push its dependencies onto the stack for DFS
        push_dependencies_no_hash_opt(db, &pkg_to_index, &dep_to_providers, &mut dep_hash_to_pkg, &visited, &mut stack, pkg);
        ...
        while let Some(pkg_index) = stack.pop() {
            ...
            push_dependencies_no_hash_opt(db, &pkg_to_index, &dep_to_providers, &mut dep_hash_to_pkg, &visited, &mut stack, index_to_pkg[pkg_index.0]);
        }
    }
...
pub fn print_unneeded_no_matrix_prealloc_dep_cache(db: &Db, alpm_packages: AlpmList<&Package>) {
    let pkg_count = alpm_packages.len();
...
pub fn print_unneeded_no_matrix_prealloc_dep_cache_no_hash_opt(db: &Db, alpm_packages: AlpmList<&Package>) {
    let pkg_count = alpm_packages.len();
...
fn compute_needed_no_matrix(db: &Db, alpm_packages: AlpmList<&Package>) -> Vec<bool> {
...
fn push_dependencies(db: &Db, pkg_to_index: &HashMap<PkgPtr, PackageIndex>, dep_to_providers: &HashMap<DepHashOpt, ProviderList>, dep_hash_to_pkg: &mut HashMap<DepHashOpt, PackageIndex>, visited: &Vec<bool>, stack: &mut Vec<PackageIndex>, pkg: &Package) {
    for hard_dep in pkg.depends() {
        // First check if there is a real package that satisfies this requirement. If not, expect the dependency to be provided as a virtual package
        if let Some(dep_pkg_index) = match dep_hash_to_pkg.entry(DepHashOpt(hard_dep.name_hash())) {
            Entry::Occupied(occupied_entry) => {
...
fn push_dependencies_no_hash_opt(db: &Db, pkg_to_index: &HashMap<PkgPtr, PackageIndex>, dep_to_providers: &HashMap<DepHash, ProviderList>, dep_hash_to_pkg: &mut HashMap<DepHash, PackageIndex>, visited: &Vec<bool>, stack: &mut Vec<PackageIndex>, pkg: &Package) {
    for hard_dep in pkg.depends() {
        // First check if there is a real package that satisfies this requirement. If not, expect the dependency to be provided as a virtual package
        if let Some(dep_pkg_index) = match dep_hash_to_pkg.entry(DepHash(hard_dep.name_hash())) {
            Entry::Occupied(occupied_entry) => {
...
mod tests {
    ...
    #[test]
    fn check_no_matrix_consistency() {
        let alpm = init_alpm();
        let (db, packages) = get_packages(&alpm);
        let matrix = DepMatrix::from_alpm_packages_no_dep_cache(packages, db);
        let needed_1 = matrix.compute_needed();
        let needed_2 = compute_needed_no_matrix(db, packages);

        assert_eq!(needed_1, needed_2);
    }
    ...
    // #[bench]
    // fn build_matrix_dep_cache(b: &mut Bencher) {
    //     let alpm = init_alpm();
    //     let (db, packages) = get_packages(&alpm);
    //     b.iter(|| {
    //         DepMatrix::from_alpm_packages_with_dep_cache(packages, db)
    //     })
    // }
    ...
    // #[bench]
    // fn compute_needed(b: &mut Bencher) {
    //     let alpm = init_alpm();
    //     let (db, packages) = get_packages(&alpm);
    //     let matrix = DepMatrix::from_alpm_packages_no_dep_cache(packages, db);
    //     b.iter(|| {
    ...
    // #[bench]
    // fn compute_needed_full(b: &mut Bencher) {
    //     let alpm = init_alpm();
    //     let (db, packages) = get_packages(&alpm);
    //     b.iter(|| {
    //         let matrix = DepMatrix::from_alpm_packages_no_dep_cache(packages, db);
    ...
    // #[bench]
    // fn compute_needed_full_no_matrix(b: &mut Bencher) {
    //     let alpm = init_alpm();
    //     let (db, packages) = get_packages(&alpm);
    //     b.iter(|| {
    //         compute_needed_no_matrix(db, packages)
    //     })
    // }


    //     #[bench]
    // fn b_print_unneeded(b: &mut Bencher) {
    //     let alpm = init_alpm();
    //     let (db, packages) = get_packages(&alpm);
    //     b.iter(|| {
    //         let matrix = DepMatrix::from_alpm_packages_no_dep_cache(packages, db);
    //         matrix.print_unneeded()
    //     })
    // }
    ...
    #[bench]
    fn b_print_unneeded_no_matrix(b: &mut Bencher) {
        let alpm = init_alpm();
        let (db, packages) = get_packages(&alpm);
        b.iter(|| {
    ...
    #[bench]
    fn b_print_unneeded_no_matrix_prealloc_dep_cache(b: &mut Bencher) {
        let alpm = init_alpm();
        let (db, packages) = get_packages(&alpm);
        b.iter(|| {
            print_unneeded_no_matrix_prealloc_dep_cache(db, packages)
        })
    }


    #[bench]
    fn b_print_unneeded_no_matrix_prealloc_dep_cache_no_hash_opt(b: &mut Bencher) {
        let alpm = init_alpm();
        let (db, packages) = get_packages(&alpm);
        b.iter(|| {
            print_unneeded_no_matrix_prealloc_dep_cache_no_hash_opt(db, packages)
        })
    }


    #[bench]
    fn b_print_unneeded_no_matrix_no_hash_opt(b: &mut Bencher) {
        let alpm = init_alpm();
        let (db, packages) = get_packages(&alpm);
        b.iter(|| {
            print_unneeded_no_matrix_no_hash_opt(db, packages)
        })
    }
